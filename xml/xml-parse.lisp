...
(defun compile-content-model (cspec &optional (cont #'cmodel-done))
  (trivia:ematch cspec
    ((and cspec (type vector))
     (alexandria:named-lambda literal (actual-name)
       (and actual-name
    ...
    ((list* 'or xs)
     (let ((options
             (mapcar (serapeum:op (compile-content-model _ cont))
                     xs)))
       (alexandria:named-lambda alternation (actual-name)
         (some (serapeum:op (funcall _ actual-name))
               options))))
    ((list '? opt)
     (let ((opt (compile-content-model opt cont)))
       (alexandria:named-lambda optional (actual-name)
         (or (funcall opt actual-name)
             (funcall cont actual-name)))))
    ((list '* repetend)
     (let (rep-cont)
       (flet ((repeat (actual-name)
                (if (or (null actual-name)
                        *left-recursion*)
                    (funcall cont actual-name)
                    (or (let ((*left-recursion* t))
                          (funcall rep-cont actual-name))
                        (funcall cont actual-name)))))
         (setf rep-cont
               (compile-content-model repetend #'repeat))
         #'repeat)))
    ((list '+ repetend)
     (compile-content-model `(and ,repetend (* ,repetend)) cont))))
